...
const messages: Messages = {
  ...
  2: ({ type, callers }) =>
    `No caller of type ${type}, only found ${Object.keys(
      callers
    )}. Make sure your caller object has a ${type} key.`,
  3: () =>
    "No origin nor server URL specified. Call createClient with an origin argument, or specify .servers[0].url in your OpenAPI specs.",
  101: () => "No body passed, but the route requires one.",
  102: ({ contentType, requestBody }) =>
    `Route does not handle content type ${contentType}, only one of ${Object.keys(
      requestBody.content
    )}`,
  103: ({ ajvError, data }) =>
    [
  ...
  105: ({ schema }) =>
    `Expected a schema, got ${schema}. Please check your specs.`,
};
