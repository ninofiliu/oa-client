const messages = {
    ...
    4: ({ path }) => `Path ${path} not found. Make sure your OpenAPI specs have a .paths['${path}'] key.`,
    5: ({ path, method, specs }) => `The path ${path} does not handle the method ${method}. Change your method to one of ${Object.keys(specs.paths[path])}, or fix your specs so as to include a .specs.paths['${path}'].${method} definition.`,
    101: () => 'No body passed, but the route requires one.',
    102: ({ contentType, requestBody }) => `Route does not handle content type ${contentType}, only one of ${Object.keys(requestBody.content)}`,
    103: ({ ajvError, data }) => [
